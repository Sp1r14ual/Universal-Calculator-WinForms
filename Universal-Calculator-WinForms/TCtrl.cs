namespace Universal_Calculator_WinForms
{
    internal class TCtrl
    {
                ...
                else
                {
                    ...
                    if (CtrlSt == TCtrlState.cValDone || CtrlSt == TCtrlState.cFuncDone)
                    {
                        ...
                        if (CheckDivZero(proc.Rop))
                        {
                            ...
                            else if (CtrlMd == TCtrlMode.fNum) return new TFrac(0, 0);
                            else return new TComplex(0, 0);
                        }
        ...
        private TANumber DoProcFunc(int func)
        {
            ...
            {
                defaultValue = new TFrac(0, 0);
                result = new TFrac(0, 0);
            }
            else
            {
                defaultValue = new TComplex(0, 0);
                result = new TComplex(0, 0);
            }
        ...
        private void ResetTCtrl()
        {
            ...
            {
                pNumber = new TFrac(0, 0);
                editor = new FEditor();
                proc.ResetTProc(2);
            }
            else
            {
                pNumber = new TComplex(0, 0);
                editor = new CEditor();
                proc.ResetTProc(3);
            }
            determ = false;
